...
"""
	A discrete-time Lotka-Volterra model with a random Gaussian fluctuation in the resource's growth rate.
	Simulated using scipy.
"""
...
if len(sys.argv) == 6:
	...
	print 'Parameter values not specified. Assigning default values (r = 1., a = 0.1, z = 1.5, e = 0.75).\n'
...
for i, Rt, Ct in z0:
	if i < EndTime-1: # If i is not the last row.
		Eps = float(sc.stats.norm.rvs(size = 1)) # A random deviate from the Gaussian distribution.
...
f1.savefig('../Results/prey_and_predators_4.pdf')
# Save figure.
